    ...
    with gr.Blocks(title='CoverGen - Politrees (v0.5)') as app:
        ...
        with gr.Tab("CoverGen"):
            ...
            with gr.Accordion('Настройки сведения аудио', open=False):
                ...
                with gr.Accordion('Эффекты', open=False):
                    with gr.Accordion('Реверберация', open=False):
                        with gr.Row():
                            ...
                            reverb_dry = gr.Slider(0, 1, value=0.9, label='Уровень сухости', info='Этот параметр отвечает за уровень исходного звука без реверберации. Чем меньше значение, тем тише звук ai вокала. Если значение будет на 0, то исходный звук полностью исчезнет.')
                ...
                with gr.Accordion('Обработка', open=False):
                    ...
                    with gr.Accordion('Фильтры', open=False):
                        with gr.Row():
                            low_shelf_gain = gr.Slider(-20, 20, value=-2.5, label='Фильтр нижних частот', info='Этот параметр контролирует усиление (громкость) низких частот. Положительное значение усиливает низкие частоты, делая звук более басским. Отрицательное значение ослабляет низкие частоты, делая звук более тонким.')
                            high_shelf_gain = gr.Slider(-20, 20, value=5, label='Фильтр высоких частот', info='Этот параметр контролирует усиление высоких частот. Положительное значение усиливает высокие частоты, делая звук более ярким. Отрицательное значение ослабляет высокие частоты, делая звук более тусклым.')
            ...
            clear_btn.click(lambda: [0, 0.5, 3, 0.25, 0.33, 128,
                                    0, 0, 0, 0.25, 0.75, 0.05, 0.9, 0.5, 0, 0,
                                    4, -16, 0, -2.5, 5, -30, 6, 10, 100, 0, 0,
